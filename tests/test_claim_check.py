...
class TestToolCallHelpers:
    ...
    def test_failed_write_does_not_count(self):
        """Codex P2 on PR #105: ToolCallCaptureHandler.on_tool_error
        still appends a record_coach_fact entry (with an "error" key —
        invalid area → 400, transient MCP failure). A failed ATTEMPT
        must not suppress the claim check or light the badge."""
        failed = [
            {"name": "get_cycle_config"},
            {
                "name": "record_coach_fact",
                "args": {"area": "Cycle.goal"},
                "error": "HTTPStatusError('400 Bad Request')",
            },
        ]
        assert not has_recording_call(failed)
        assert recorded_areas(failed) == []

    def test_mixed_failed_and_successful_writes(self):
        """One errored + one successful → the turn counts as recorded,
        but only the successful area reaches the badge."""
        calls = [
            {
                "name": "record_coach_fact",
                "args": {"area": "Cycle.goal"},
            ...
            {
                "name": "record_coach_fact",
                "args": {"area": "Profile.devices"},
                "result": '{"action":"created"}',
            },
        ]
        assert has_recording_call(calls)
        assert recorded_areas(calls) == ["Profile.devices"]
...
class TestEnforceRecordClaim:
    ...
    def test_correction_with_failed_write_still_warns(self, tmp_chat_db):
        """Codex P2 end-to-end: the correction round ATTEMPTS the write
        but it errors (entry carries "error"), and the text still
        claims success → not fixed, warning appended. A failed attempt
        is not proof."""
        coach = _coach(tmp_chat_db)

        async def _failed_write(user_input, thread_id, ctx, tool_calls_sink=None):
            tool_calls_sink.append(
                {
                    "name": "record_coach_fact",
                    "args": {"area": "Cycle.goal"},
        ...
        coach._run_turn_inner = _failed_write
        trace = _trace()
        out = asyncio.run(
            coach._enforce_record_claim("已记录。", "t", [], trace)
        )
        assert WARNING_LINE in out
        assert trace.extras["claim_check"]["corrected"] is False
        assert trace.extras["claim_check"]["areas"] == []
...
def _msg(
    ...
    # production walk only consumes real lists / real ids, so set
    # explicitly.
...
class TestHistoryFactsRecorded:
    def test_facts_attach_to_turns_final_text_message(self, tmp_chat_db):
        """ReAct turn: tool-calling ai msg (no text) → confirming tool
        msg → final text ai msg. The areas written land on the FINAL
        message, and only after the ToolMessage confirms success."""
        ...
        tool = _msg("tool", '{"action":"created"}', tool_call_id="x")
    ...
    def test_failed_write_does_not_light_badge(self, tmp_chat_db):
        """Codex P2, history side: AIMessage.tool_calls is the model's
        REQUEST — if the ToolMessage outcome is status='error', the
        write didn't happen and the badge must not light."""
        h = _msg("human", "记一下")
        ai_tool = _msg(
            "ai",
            "",
        ...
        tool = _msg("tool", "Error: 400 Bad Request", tool_call_id="x", status="error")
        ai_final = _msg("ai", "好的，处理完了。")
        tuples = [StubTuple(_ckpt("2026-06-10T01:00:00Z", [h, ai_tool, tool, ai_final]))]
        coach = _coach_with(tmp_chat_db, tuples)
    ...
    def test_unconfirmed_request_does_not_light_badge(self, tmp_chat_db):
        """A write REQUEST with no ToolMessage outcome at all (crash
        mid-turn) stays unconfirmed — conservative: no badge."""
        h = _msg("human", "记一下")
        ai_tool = _msg(
            "ai",
            "",
            tool_calls=[
                {"name": "record_coach_fact", "args": {"area": "Cycle.goal"}, "id": "x"}
            ],
        )
        ai_final = _msg("ai", "已记录。")
        tuples = [StubTuple(_ckpt("2026-06-10T01:00:00Z", [h, ai_tool, ai_final]))]
        coach = _coach_with(tmp_chat_db, tuples)
    ...
    def test_double_lie_is_durably_marked(self, tmp_chat_db):
        """PR #105 review P2: WARNING_LINE only lived in the streamed /
        returned text — a reload showed the double-lie turn clean. The
        sentinel human message IS checkpointed though, so the history
        walk can re-derive the verdict: correction round happened + no
        successful write + the answer still claims one →
        claim_unverified on the turn's final ai message. Durable twin
        of the ✓ badge."""
        h = _msg("human", "记一下纽马")
        lie1 = _msg("ai", "我已将以下信息更新至你的档案。")
        corr = _msg("human", CORRECTION_PROMPT)
        lie2 = _msg("ai", "我已经为你记录在案。")
        tuples = [StubTuple(_ckpt("2026-06-10T01:00:00Z", [h, lie1, corr, lie2]))]
        coach = _coach_with(tmp_chat_db, tuples)

        out = coach.get_history_with_ts("t")

        ais = [r for r in out if r["role"] == "ai"]
        # The post-correction lie is flagged; the pre-correction one is
        # not (the flag marks the turn's final verdict, the UI shows it
        # under the last bubble).
        assert ais[-1].get("claim_unverified") is True
        assert "claim_unverified" not in ais[0]

    def test_corrected_write_is_not_marked(self, tmp_chat_db):
        """The correction round performs a REAL confirmed write → badge,
        no claim_unverified."""
        h = _msg("human", "记一下纽马")
        lie = _msg("ai", "我已将信息更新至你的档案。")
        corr = _msg("human", CORRECTION_PROMPT)
        ai_tool = _msg(
            "ai",
            "",
            tool_calls=[
                {"name": "record_coach_fact", "args": {"area": "Cycle.goal"}, "id": "y"}
            ],
        )
        tool = _msg("tool", '{"action":"created"}', tool_call_id="y")
        ai_final = _msg("ai", "已记录：纽马 2026-11-01。")
        tuples = [
            StubTuple(_ckpt("2026-06-10T01:00:00Z", [h, lie, corr, ai_tool, tool, ai_final]))
        ]
        coach = _coach_with(tmp_chat_db, tuples)

        out = coach.get_history_with_ts("t")
        final = [r for r in out if r["role"] == "ai" and r["content"]][-1]
        assert final["facts_recorded"] == ["Cycle.goal"]
        assert "claim_unverified" not in final

    def test_honest_retraction_is_not_marked(self, tmp_chat_db):
        h = _msg("human", "记一下")
        lie = _msg("ai", "已记录。")
        corr = _msg("human", CORRECTION_PROMPT)
        retract = _msg("ai", "更正：该信息尚未被写入档案。")
        tuples = [StubTuple(_ckpt("2026-06-10T01:00:00Z", [h, lie, corr, retract]))]
        coach = _coach_with(tmp_chat_db, tuples)

        out = coach.get_history_with_ts("t")
        assert all("claim_unverified" not in r for r in out)

    def test_flag_is_sentinel_gated_not_retroactive(self, tmp_chat_db):
        """Ordinary turns claiming a (possibly stale-but-true) recording
        with no correction round are NOT flagged — pre-enforcement
        history (e.g. the original 5/30 lie) is not retro-judged, and a
        true claim about a previous turn never earns a permanent ⚠️."""
        h1 = _msg("human", "记一下")
        old_lie = _msg("ai", "我已将信息更新至你的档案。")  # pre-enforcement turn
        h2 = _msg("human", "你记了吗？")
        stale_true = _msg("ai", "是的，已记录在案。")
        tuples = [
            StubTuple(_ckpt("2026-06-10T01:00:00Z", [h1, old_lie, h2, stale_true]))
        ]
        coach = _coach_with(tmp_chat_db, tuples)
