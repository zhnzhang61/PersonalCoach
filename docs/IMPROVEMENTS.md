...
|---|---|---|
| `data_processor.py` | `tests/test_data_processor.py` ✅ pass-1 + pass-2 done 2026-05-27 | **Pass 1** (2026-05-12, 81 tests): RunActivity from_garmin + derived props, ManualActivity round-trip, `_bucket_run_surface`, DataProcessor bootstrap on tmp_path, semantic memory CRUD, training blocks CRUD + date validation, manual activity CRUD, `calculate_category_stats` (perceived-stream derivation), `compute_telemetry_summary` (pandas pure fn). **Pass 2** (2026-05-27, +31 tests = 112 total): Garmin-file-dependent paths. `compile_health_ledger` (5: empty dir / sleep+rhr+hrv join / activity miles aggregation / CSV persistence / hrvData-overrides-summary precedence). `get_hr_zones` (5: missing file / 5-zone parse+sort / `<N` and `>N` sentinel bounds / unparseable-range skip / unknown-name literal rpe_label fallback). `get_athlete_profile_full` (7: empty userData / weight g→kg / age from birthDate / LT pace dm/s vs m/s conversion / sanity-range clamp / current_block phase computation / hr_zones threaded through). `get_readiness` (6: empty-ledger unknown branch / green when in band + sleep ≥7 / red on HRV >10% drop / red on RHR >10% spike / red on sleep <5h / yellow otherwise). `get_training_load` (4: empty state / non-running filter / ACWR "sweet" band at ratio 1.0 / weekly miles bucket by Monday). `compute_cycle_and_week_stats` (3: unknown block_id / empty block keeps shape / one run aggregates into cycle + week). |
...
| `google_calendar.py` | `tests/test_google_calendar.py` ✅ done 2026-05-27 | 35 tests covering: insecure-localhost env-flip gating (5), `_client_config` env-var paths (4), `GoogleCalendar.__init__` token-dir bootstrap (3), `authorization_url` + PKCE-verifier stashing (3), `finish_flow` token exchange + verifier pop (2), `_load_creds` refresh-on-expired path (4), `is_connected` corruption tolerance (3), `disconnect` idempotency (2), `list_events` not-connected error + HttpError wrap + 6 event-mapping shape assertions (all-day vs timed, missing summary, missing items key, custom calendar_id, list-call kwargs pinning). Mocks `Flow.from_client_config`, `Credentials.from_authorized_user_file`, and `googleapiclient.discovery.build` — no network. |
