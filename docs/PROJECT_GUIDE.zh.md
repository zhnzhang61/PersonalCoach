...
- [3. 后端](#3-后端)
  ...
  - [3.4 AI / Coach](#34-ai--coach)
    ...
    - [3.4.5 计划中 — 档案 + 周期配置采集](#345-计划中--运动员档案-a--周期配置-b-采集) — A/B intake 进 CME
...
- **PR-2（行为变更）** — 3 个 MCP 工具（`record_coach_fact`、
  ...
  接线（`make_plan` 的 prefetch 读 profile + cycle config）+ 测试。prompt
  这一半写好先拿给你过目再 merge。
...
- **运动员档案（A）+ 周期配置（B）采集** — 下一个高杠杆的 build：把 A/B
  ...
  完整 spec 见 [§3.4.5](#345-计划中--运动员档案-a--周期配置-b-采集)。
