...
# ---------------------------------------------------------------------------
# Prompt version. Bump on any system-prompt edit + add a row to
# docs/PROMPT_CHANGELOG.md. The traces emitted per turn carry both the
# version label and a short content hash so we can grep "all turns on
# v7" or detect "the prompt drifted between v7 and the actual content
...
class AgenticCoach:
    ...
    async def chat_stream(
        ...
        **Cross-loop bridge** (codex P1 catch on PR #73):
        AsyncSqliteSaver + MCP client + LangGraph agent all bind to
        `self._loop` (the daemon-thread loop) the first time
        `_ensure_agent()` runs there. Driving `astream_events` from
        the request's ASGI loop instead would (a) initialize the
        resources on the request loop if the stream is first, leaving
        the next sync `chat()` call's `_submit` to use them from the
        wrong (background) loop; or (b) cross-loop access the existing
        bg-loop-bound resources if sync calls came first. Both
        manifest as "no current event loop" / "Task got Future
        attached to different loop" failures.

        Fix: schedule the actual `astream_events` drive on
        `self._loop` via `run_coroutine_threadsafe`. The producer
        pushes events into an `asyncio.Queue` that lives on the
        consumer's loop (the request handler's loop), using
        `consumer_loop.call_soon_threadsafe(queue.put_nowait, ev)`
        for the cross-loop hop. This keeps every coroutine that
        touches `self._agent` / saver / MCP on `self._loop`, matching
        the contract _submit enforces for sync callers.

        The full message state (including tool calls) still lands in
        the checkpointer at end — frontend should invalidate
        `/api/ai/history` on `done` to pick up the canonical history
        with timestamps.
        """
        ...
        async def _producer() -> None:
            """Runs on self._loop. Drives astream_events to completion
            and forwards filtered events onto the consumer's loop."""

            def _emit(ev: object) -> None:
                # Cross-loop hop: schedule a put on the consumer's
                # queue from this thread (self._loop's thread).
                consumer_loop.call_soon_threadsafe(out_queue.put_nowait, ev)

            # Trace lives inside the producer (same loop as the
            # LangGraph drive) so duration_ms is wall-clock time of
            # the actual turn, not request-loop time. final_answer
            # accumulates from tokens we emit.
            assembled_answer = []
            with self.tracer.turn(
                ...
                try:
                    ...
                    # message in the checkpointer and the streamed
                    # assembly stay consistent.
                    ...
                    assembled_answer.append(footer)
                    _emit({"type": "token", "content": footer})
                    _emit({"type": "done"})
                    trace.final_answer = "".join(assembled_answer)
                except Exception as e:
                    _emit({"type": "error", "message": str(e)})
                    # The tracer's context manager will capture the
                    # exception into trace.error and write the row,
                    # but the SSE consumer also needs the error event
                    # before we exit. The `raise` here propagates into
                    # the context manager.
                    raise
                finally:
                    _emit(sentinel)

        # Schedule the producer on the agent-owned loop. The returned
        # Future is concurrent.futures.Future; we only use it to
        # best-effort cancel on consumer drop.
        producer_future = asyncio.run_coroutine_threadsafe(
            _producer(), self._loop
        )
        ...
        try:
            ...
            # Consumer abandoned mid-stream (GeneratorExit, client
            # disconnect, etc.) — try to cancel the underlying
            # LangGraph turn. cancel() is best-effort: it works
            # before the task starts and at await-points, but won't
            # interrupt synchronous LLM provider code mid-call. The
            # turn may still finish + commit to checkpointer in the
            # background, which matches prior behavior for sync chat.
    ...
    async def _action_turn(
        ...
        *,
        ...
        """Run a prebuilt-action turn: prefetch + LangGraph drive.
        `kind` identifies which action for the trace row (e.g.
        'action.review_workout'). The inner _run_turn is called with
        _skip_trace=True so we get ONE trace row covering prefetch +
        LLM, not two."""
        with self.tracer.turn(
            kind=kind,
            thread_id=thread_id,
