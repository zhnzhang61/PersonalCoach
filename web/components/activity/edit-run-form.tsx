...
export function EditRunForm({ run, onClose }: Props) {
  ...
  // would permanently record the Garmin default name and week 0. Hold
  // Save until the suggestion settles (error settles too: the fallback
  // chain then applies knowingly, not by race).
  const suggestionPending =
    !hasSavedName && nameEdit === null && suggestQuery.isLoading;
